...
- `--without-metadata`
  ...
  - デフォルトは `false`
- `-e` , `--embedded`
  - 画像をデータURLとしてメタデータの `image` に埋め込む
  - デフォルトは `false`
  - メタデータの配置先にIPFSを使う場合に有用
...
留意すべきは、この設定では `quantity` の合計である3,000丁度の生成でなければ超過分(3,001以降)は `probabilities` の設定に依存して抽選され、不足分は `quantity` で設定した確定抽選枠を使い切らずに完了してしまう。  
なので、通常はこの設定においては、`summon` コマンドを実行する際、 `-q` の引数には3,000を渡すのが基本的な利用方法となる。
...
もう一つ気をつけるべきことがある。  
 `-f` (強制上書き)オプションを指定しない場合においては、出力先のフォルダにすでに生成された画像が存在しないもの(欠番)だけを生成するが、その際はすでに生成されているものの出現数を考慮しないため、新たに生成する分だけで確定抽選枠から新たに選ばれる。

すなわち、 `allocations` を設定する場合は `-f` は指定すべきである。
